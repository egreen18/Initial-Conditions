...
%% Section 3.1 - FSP Test
% Make a copy of the model for FSP
Model_FSP_test = Model;

% Set the times at which distributions will be computed:
Model_FSP_test.tSpan = linspace(0,40,200);

% Set the solution scheme to FSP
Model_FSP_test.solutionScheme = 'FSP';

% Set the FSP 1-norm error tolerance
Model_FSP_test.fspOptions.fspTol = 1e-4;
...
% Enable steady state initial distribution approximation
% NOTICE - steady state approximation sets the model to equilibrium at t0
Model_FSP_test.fspOptions.initApproxSS = false;

% Create symbolic propensity functions
Model_FSP_test = Model_FSP_test.formPropensitiesGeneral('Model_FSP_dOpt_2',false);
...
prob = Model_FSP.Solutions.fsp{end}.p.data.vals;

gate_prob = prob(gate);   
gate_prob = gate_prob/sum(gate_prob);

Model_FSP_test.initialCondition = state(:,gate)-1; % Minus one for 0-index
Model_FSP_test.initialProbs = gate_prob;

% Solve Model
Model_FSP_test.Solutions = Model_FSP_test.solve;

% Plot marginal distributions at given times
timeInd = [1,50,100,200];
Model_FSP_test.plotFSP(plotType='marginals', indTimes = timeInd)
...
gate_prob = [gate_prob, zeros(1,6)];        % Appending 6 zeros for the constraint
...
gate_sens = [gate_sens,zeros(4,6)];         % Appending 6 zeros for the constraint

% Flattening the gate_sens matrix
gate_sens = gate_sens(:);

% Normalize the sensitivity matrix to the probability matrix
% NOTE - Should this be normalized to sum to zero? If so, normalize by
% subtracting the average from each value on a per parameter basis
...
Model_gate.fspOptions.verbose = true;
